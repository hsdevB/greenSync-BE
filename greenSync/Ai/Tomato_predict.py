...
SELECT
    ...
    h."createdAt" AS humidity_created_at -- humidity 테이블의 createdAt 컬럼 추가
FROM "Farm" f
...
def preprocess_data(df):
    df.rename(columns={
        ...
        "extn_tprt": "EXTN_TPRT",
        "extn_srqt": "EXTN_SRQT",
        "humidity_created_at": "HUMIDITY_CREATED_AT" # 새로 추가된 컬럼 이름도 rename
    }, inplace=True)
    ...
    # humidity 테이블의 createdAt 시간 정보를 사용하도록 수정
    if 'HUMIDITY_CREATED_AT' in df.columns and pd.notnull(df['HUMIDITY_CREATED_AT'].iloc[0]):
        ...
        # humidity_created_at이 없거나 NULL인 경우 현재 시간 사용 (폴백)
        now = datetime.now()
        df["year"] = now.year
        df["month"] = now.month
        df["day"] = now.day
...
def save_to_db(farm_id, control_values):
    try:
        ...
        cur.execute(insert_sql, (
            ...
            datetime.now(), # createdAt에 현재 시간 저장
            datetime.now()  # updatedAt에 현재 시간 저장
        ))
        conn.commit()
        cur.close()
        conn.close()
