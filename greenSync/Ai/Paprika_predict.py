...
def load_models():
    try:
        models = {
            'qty': joblib.load(os.path.join(BASE_DIR,"Paprika","WTSPL_QTYpaprika_model.pkl")),
            'vnt': joblib.load(os.path.join(BASE_DIR,"Paprika","VNTILAT_TPRT_1paprika_model.pkl")),
            'htg': joblib.load(os.path.join(BASE_DIR,"Paprika","HTNG_TPRT_1paprika_model.pkl")),
            'yield': joblib.load(os.path.join(BASE_DIR,"Paprika","paprika_yield_model.pkl"))
        }
...
SELECT
    ...
    n."phLevel" AS ntslt_spl_ph_lvl,
    n."elcDT" AS ntslt_spl_elcdt,
    w."isDay" AS dytm_night_cd,
    w."isRain" AS prcpt_yn,
    w."windDirection" AS wndrc,
    w."windSpeed" AS wdsp,
    w."outsideTemp" AS extn_tprt,
    w.insolation AS extn_srqt,
    h."createdAt" AS humidity_created_at
FROM "Farm" f
LEFT JOIN "CarbonDioxide" cd ON f.id = cd."farmId"
LEFT JOIN "Temperature" t ON f.id = t."farmId"
LEFT JOIN "Humidity" h ON f.id = h."farmId"
LEFT JOIN "Nutrient" n ON f.id = n."farmId"
LEFT JOIN "Weather" w ON f.id = w."farmId"
WHERE f.id = %s
...
def preprocess_data(df):
    df.rename(columns={
        ...
        "extn_tprt": "EXTN_TPRT",
        "extn_srqt": "EXTN_SRQT",
        "humidity_created_at": "HUMIDITY_CREATED_AT"
    }, inplace=True)
    ...
    if 'HUMIDITY_CREATED_AT' in df.columns and pd.notnull(df['HUMIDITY_CREATED_AT'].iloc[0]):
        humidity_timestamp = pd.to_datetime(df['HUMIDITY_CREATED_AT'].iloc[0])
        df["year"] = humidity_timestamp.year
        df["month"] = humidity_timestamp.month
        df["day"] = humidity_timestamp.day
        df["hour"] = humidity_timestamp.hour
    else:
        now = datetime.now()
        df["year"] = now.year
        df["month"] = now.month
        df["day"] = now.day
        df["hour"] = now.hour
        sys.stderr.write("경고: HUMIDITY_CREATED_AT 데이터가 없거나 유효하지 않아 현재 시간을 사용합니다.\n")
...
def predict_control_values(df, models):
    ...
    try:
        ...
        results['WTSPL_QTY'] = round(opt_qty / 10, 2) # 급수량을 10으로 나눔
        sys.stderr.write(f"예측 급수량(WTSPL_QTY): {results['WTSPL_QTY']:.2f}\n")
    except Exception as e:
...
def predict_yield(df, control_values, model_yield):
    try:
        ...
        df_input = df.copy()
        # predict_control_values에서 이미 10으로 나눈 값이 넘어오므로, 여기서 다시 나눌 필요 없음
...
def save_to_db(farm_id, control_values):
    try:
        ...
        INSERT INTO "Optimization" (
            "farmId", "heating", "fan", "waterSupply", "createdAt", "updatedAt"
        ) VALUES (%s, %s, %s, %s, %s, %s);
        """
        cur.execute(insert_sql, (
            ...
            control_values['WTSPL_QTY'], # predict_control_values에서 이미 10으로 나눈 값이 넘어옴
            datetime.now(),
            datetime.now()
        ))
        conn.commit()
